...
test.describe('Test Endpoint /influencer/influencer/updateInfluencerDraft', () => {
	...
	test('Endpoint Test /influencer/influencer/updateInfluencerDraft without auth', async ({ request }) => {
		const Issues = await request.get('/influencer/influencer/updateInfluencerDraft');
		expect.soft(Issues.status(401)).toBe();
	});


	test('Endpoint Test /influencer/influencer/updateInfluencerDraft', async ({ request }) => {
		const Issues = await request.put('/influencer/influencer/updateInfluencerDraft',{data: requestBody,headers:{Authorization:`Bearer ${auth}`}} );
