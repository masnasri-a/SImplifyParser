...
test.describe('Test Endpoint /campaign/verifyProofOfInsight?subCampaignId=2122&isProofOfInsightValid=true', () => {
	const auth = AUTH
	const expectResult = {};
	...
	test('Endpoint Test v1/campaign/verifyProofOfPost Response - 200 OK', async ({ request }) => {
		...
		const setContent = await request.post('https://internal-api.estidar.com/api/v1/campaign/setProofOfInsight?subCampaignId=' + subcampaigns, {
			data: ["SUBCAMPAIGN/1317/DRAFT/1694160797879-345658286_970115157328312_3782093863021317392_n_waifu2x_photo_noise1_scale.png"], headers: {
				'Auth-Token': TOKEN
			}
		})
	...
	test('Endpoint Test v1/campaign/verifyProofOfInsight Response - 200 OK', async ({ request }) => {
		const Issues = await request.put('https://api.estidar.com/api/v1/campaign/verifyProofOfInsight?subCampaignId='+subcampaigns+'&isProofOfInsightValid=true', { headers: { Authorization: `Bearer ${auth}` } });
		expect.soft(Issues.status()).toBe(200);

	});

	test('Endpoint Test v1/campaign/verifyProofOfInsight Response - 400 Bad Request', async ({ request }) => {
		const Issues = await request.put('https://api.estidar.com/api/v1/campaign/verifyProofOfInsight', { headers: { Authorization: `Bearer ${auth}` } });
	...
	test('Endpoint Test v1/campaign/verifyProofOfInsight Response - 401 Forbidden', async ({ request }) => {
		const Issues = await request.put('https://api.estidar.com/api/v1/campaign/verifyProofOfInsight?subCampaignId=285&isProofOfInsightValid=true');
