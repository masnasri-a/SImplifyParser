...
test.describe('Test Endpoint /campaign/removeSubCampaign?subCampaignId=367', () => {
	...
	test('Endpoint Test v1/campaign/removeSubCampaign Response - 200 OK', async ({ request }) => {
		console.log(subcampaigns);
		const Issues = await request.delete('https://api.estidar.com/api/v1/campaign/removeSubCampaign?subCampaignId='+subcampaigns, { headers: { Authorization: `Bearer ${auth}` } });
		expect.soft(Issues.status()).toBe(200);
		const DataTypeParser: Record<string, any> = {}
		const jsonResult = await Issues.json()
		for (const key in jsonResult['subCampaigns']) {
			expect.soft(typeof jsonResult[key]).toBe(DataTypeParser[key.toLowerCase()])
		}
	});

	test('Endpoint Test v1/campaign/removeSubCampaign Response - 400 Bad Request', async ({ request }) => {
		const Issues = await request.delete('https://api.estidar.com/api/v1/campaign/removeSubCampaign?subCampaignId='+subcampaigns, { headers: { Authorization: `Bearer ${auth}` } });
	...
	test('Endpoint Test v1/campaign/removeSubCampaign Response - 403 Forbidden', async ({ request }) => {
