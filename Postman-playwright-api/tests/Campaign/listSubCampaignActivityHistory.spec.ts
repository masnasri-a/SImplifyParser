...
test.describe('Test Endpoint /campaign/listSubCampaignActivityHistory?subCampaignId=2122', () => {
	const auth = AUTH
	const expectResult = {};
	const sortedKeyResult = Object.keys(expectResult).sort();

	test('Endpoint Test v1/campaign/listSubCampaignActivityHistory Response - 200 OK', async ({ request }) => {
		const Issues = await request.get('https://api.estidar.com/api/v1/campaign/listSubCampaignActivityHistory?subCampaignId=2122',{headers:{Authorization:`Bearer ${auth}`}} );
	...
	test('Endpoint Test v1/campaign/listSubCampaignActivityHistory Response - 401 Unauthorize', async ({ request }) => {
